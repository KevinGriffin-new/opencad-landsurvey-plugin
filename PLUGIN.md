...
| `LS_IMPORTPLAN <path>` | ribbon file dialog | Import recognized plan geometry (the `plan2cad` / `plat2json` JSON) — lines / arcs / circles / texts / polylines onto their named layers. Ordered chains (`polylines`, alias `plines`) become LWPolylines (closed when the last point repeats the first); flattened line segments that duplicate a chain's edges are skipped. |
